...
alter table dev.proyecto_configuracion_ui
  add column if not exists lot_state_fill_opacity numeric(4,3) not null default 0.14;

update dev.proyecto_configuracion_ui
   set lot_state_fill_opacity = 0.14
 where lot_state_fill_opacity is null;

alter table dev.proyecto_configuracion_ui
  drop constraint if exists proyecto_configuracion_ui_lot_state_fill_opacity_check;

alter table dev.proyecto_configuracion_ui
  add constraint proyecto_configuracion_ui_lot_state_fill_opacity_check
  check (lot_state_fill_opacity >= 0 and lot_state_fill_opacity <= 1);
