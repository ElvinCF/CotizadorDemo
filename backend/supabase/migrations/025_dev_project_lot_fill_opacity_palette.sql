...
alter table dev.proyecto_configuracion_ui
  add column if not exists lot_state_fill_opacity_palette jsonb not null default '{
    "disponible": 0.14,
    "separado": 0.14,
    "vendido": 0.14,
    "selected": 0.14
...
update dev.proyecto_configuracion_ui
   set lot_state_fill_opacity_palette = jsonb_build_object(
     'disponible', coalesce(lot_state_fill_opacity, 0.14),
     'separado', coalesce(lot_state_fill_opacity, 0.14),
     'vendido', coalesce(lot_state_fill_opacity, 0.14),
     'selected', coalesce(lot_state_fill_opacity, 0.14)
   )
 where lot_state_fill_opacity_palette is null
    or lot_state_fill_opacity_palette = '{}'::jsonb;
...
select
  ...
  ) as lot_state_palette,
  coalesce(pcu.lot_state_fill_opacity, 0.14) as lot_state_fill_opacity,
  coalesce(
    pcu.lot_state_fill_opacity_palette,
    jsonb_build_object(
      'disponible', coalesce(pcu.lot_state_fill_opacity, 0.14),
      'separado', coalesce(pcu.lot_state_fill_opacity, 0.14),
      'vendido', coalesce(pcu.lot_state_fill_opacity, 0.14),
      'selected', coalesce(pcu.lot_state_fill_opacity, 0.14)
    )
  ) as lot_state_fill_opacity_palette,
  pcu.logo_header_url,
  pcu.logo_footer_url,
  pcu.mapa_svg_url,
  pcu.mapa_webp_url,
